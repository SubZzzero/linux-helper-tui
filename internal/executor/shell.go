...
// ExecuteShellStreaming launches a shell-execution recipe with live output.
func ExecuteShellStreaming(ctx context.Context, runner StreamingCommandRunner, recipe models.Recipe, values map[string]string, sink OutputSink) (models.ExecutionResult, error) {
	command := recipe.Command
	for key, value := range values {
		command = strings.ReplaceAll(command, "{{"+key+"}}", shellQuote(value))
	}
	...
	result, err := runner.RunShellStreaming(ctx, command, sink)
	if err != nil {
		return result, fmt.Errorf("execute shell recipe %q: %w", recipe.ID, err)
	}
