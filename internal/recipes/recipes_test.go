...
func TestEmbeddedRecipeCorpusLoads(t *testing.T) {
	...
	loaded, err := loader.Load()
	require.NoError(t, err)
	assert.GreaterOrEqual(t, len(loaded), 48)
	...
	assert.GreaterOrEqual(t, categoryCounts["filesystem"], 8)
	assert.GreaterOrEqual(t, categoryCounts["environment"], 8)
	assert.GreaterOrEqual(t, categoryCounts["network"], 8)
	assert.GreaterOrEqual(t, categoryCounts["system"], 8)
	assert.GreaterOrEqual(t, categoryCounts["text"], 8)
	assert.GreaterOrEqual(t, categoryCounts["users"], 8)
}
